...
3. [OK] Запастить расчет через процессор отдельный
    ...
    0.  Убрать тормоза при отпраке Weight.
        ...
        0. Разделить память Net для слоев на расчетную и обменную.
            1. Варианты решения:
                1. MultiNet - у каждого потока свой net и один в главном потоке с которым все синхронятся.
                    1. Net основного потока при загрузке формирует гуд
                    0. Каждый участник 
                        1. при создании создает свой персональный net.
                ...
                0. HyperNet - net у всех общий но каждый поток получает персональный срез данных через net. - Не можем придумать разрешение потока в указатель памяти pointer=f(thread).
                0. BigNet - net общий но при этом в нем предусмотрены методы для всех участников для работы с данынми. - Добавление участника приведет к изменению кода.
                0. PairNet - у каждого участника один net имеющий основную и кэш память. - Нельзя объединять разных участников в одном приложении.
