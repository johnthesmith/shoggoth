...
Тичер медленно подсовывает тестовые задания так как проц быстро их обсчитывает. Проц работает вхолостую
У нас нет понятимания тирером завершил ли процессор рассчет.
Возможно нужен критерий остановки расчета процессора что бы он не лупил в холостую. 
Возможно нужен лимит еррора или иного слоя до которого проц считает. как только лимит менее... то проц останавливается и ждет нового задания.
Нужен алгоритм завершения теста. Скорее всего на базе errorsBeforeChange.
