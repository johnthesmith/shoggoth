...
```mermaid
flowchart LR
    subgraph "Приложение Брэйн (Brain)"
        direction LR
    ...
    subgraph "Приложение Тичер (Teacher)"
        direction LR
    ...
    subgraph "Приложение Эволюция"
        E["Компонент Эволюция\n(сбор метрик, критерии)"]
    end
    ...
    %% Взаимодействия
    T_Тичер -- "1. Запрос состояния<br>(синхронный)" --> P_Сервер
    P_Сервер -- "2. Ответ (последний снимок)" --> T_Тичер
    T_Тичер -- "3. Отправка новых слоёв" --> P_Сервер

    P_Процессор -- "4. Результаты расчёта<br>(асинхронная выгрузка)" --> P_Сервер

    P_Сервер -- "Чтение метрик" --> E
    E -- "6. Команда 'пересоздать процессор'" --> P_Сервер

    P_Сервер -- "Чтение состояния (отложено)" --> U

    %% Внутренние взаимодействия Брэйн
    P_Сервер -- "Запись новых слоёв" --> P_Процессор
    P_Сервер -- "Команда пересоздания" --> P_Процессор
